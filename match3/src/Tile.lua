...
function Tile:init(x, y, color, variety)
    ...
    --Assignment: Giving shininess to the tiles
    ...
    self.shinyTable = {timer = nil, factor = 1}
end
...
function Tile:render(x, y)
    ...
    love.graphics.draw(gTextures['main'], gFrames['tiles'][self.color][self.variety],
        self.x + x, self.y + y)
    --Assignment: Making the shiny tile glow
    if self.shiny then
        love.graphics.setColor(1, 1, 1, self.shinyTable.factor)
        ...
        if not self.shinyTable.timer then
            self.shinyTable.timer = Timer.tween(0.5, {
                ...
                Timer.tween(0.5, {
                    [self.shinyTable] = {factor = 120/255}
                }):finish(function()
