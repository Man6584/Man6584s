...
function GenerateQuadsBricks(atlas)
    --since the location of the locked brick in the spritesheet
    --is awkward, we'd better add it separately from all other bricks
    local Bricks = table.slice(GenerateQuads(atlas, 32, 16), 1, 21)
    table.insert(Bricks, love.graphics.newQuad(32*5, 16*3, 32, 16, atlas:getDimensions()))
    return Bricks
end
...
function GenerateQuadsPowerUps(atlas)
    --Initial location of the first powerups sprite
    local x = 0
    local y = 192
    ...
    --just like other elements, the location of the powerups is
    --repetitive as they're near each other. We just need to
    --do it 9 times since there are 9 different sprites for powerups
    for i = 0, 9 do
