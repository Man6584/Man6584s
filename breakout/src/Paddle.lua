...
function Paddle:update(dt)
    ...
    --Assignment
    --Changing our width everytime psize changes
    --this prevents visual glitch and prevent our paddle to
    --go pass the screen limit
    if psize == 4 then
        self.width = 128
    elseif psize == 3 then
        self.width = 96
...
function Paddle:render()
    --Assignment
    --Change self.size to psize since we'll be changing the value of psize in playstate
    love.graphics.draw(gTextures['main'], gFrames['paddles'][psize + 4 * (self.skin - 1)],
        self.x, self.y)
end