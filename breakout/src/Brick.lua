...
function Brick:hit(hasKey)
    ...
    --Assignment: same thing but with lock now
    if self.locked and hasKey then
        ...
        --more particles if we're unlocking the locked
        ...
        --removing the locks
        self.locked = false
    --nothing happens without the keys
    elseif self.locked then
        gSounds['locked']:play()
    --goes normally for other bricks
    else
        self.psystem:setColors(
            paletteColors[self.color].r / 255,
           paletteColors[self.color].g / 255,
...
function Brick:render()
    --If we randomized the bricks to generate as locked,
    --we'll draw the 22nd index (last index) of the brick quads
